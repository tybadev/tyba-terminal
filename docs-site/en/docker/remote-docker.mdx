...
This is the good use case for the whole thing: you're in an SSH session on the VPS, you open the panel, and it is already listing the containers **from over there**.
...
Local workspace, Local target. VPS workspace, VPS target. The selector is there for when you want to leave the default.
...
That is on purpose: it **does not hijack your SSH pane**. Mixing the two would make the `sh: postgres` on your Mac look exactly like the VPS's, and confusing those two is expensive.
...
The reason: those actions depend on project paths **on this machine**, and the VPS's path doesn't exist here. For remote compose, use an SSH pane and run `docker compose` there.
</Warning>
...
The remote refresh pays for a handshake, the network and, on the first connection, **your approval in 1Password**. The local timeout measures a Docker that answers in milliseconds; the remote one gets a lot more slack, because with the local ceiling the remote panel only ever delivered an empty list.
...
<Note>
**Availability is cached per target.** Docker on your Mac says nothing about Docker on the VPS, so each target has its own state — and the panel doesn't keep re-asking every time you glance at it.
</Note>
