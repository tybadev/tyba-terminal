...
Translated into what you feel: the **first** connection to a host authenticates; the split, the new tab and every query from the Docker panel **reuse the same connection**. Without this, your 1Password or `ssh-agent` would ask for approval again on every `docker ps`. Authenticating once is normal; per command is a bug.
