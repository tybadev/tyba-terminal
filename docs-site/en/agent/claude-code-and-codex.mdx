...
Typing `claude` into a shell is a different thing: no worktree is created, and the agent runs in whatever folder the shell is in. On Linux, in bash or zsh, TYBA still starts that `claude` with a gate — and with a jail, if the folder is inside a git repository in your home. In the other cases the agent starts with no gate, and TYBA notices and offers to reopen it managed — see [Agent started in a shell](/en/agent/agent-in-a-shell).
</Note>
