...
| Command history | `⌘R` | `Ctrl+Shift+R` |
...
TYBA draws its own menu bar — **Tyba, File, Edit, View, Session, Window and Help** — and the shortcuts shown there are the ones on this page: the bar reads your configuration, not a second table. Remapping a shortcut changes what the menu displays.

<Note>
**There is no menu bar on Windows or Linux.** Both run with TYBA's own title bar, and there is nowhere to draw one. Nothing is lost: everything in the menu has a shortcut or lives in the palette.
</Note>

Terminal shortcuts — copy, paste, find, select all — are deliberately **not** in the menu. On macOS a menu accelerator is consumed by the system before it reaches the terminal: putting `⌘C` there would switch the key off inside the session.
