...
Closing something with a **running agent** opens a confirmation, with **Close anyway** and **Show what is running**. The agent does not die until you choose: closing mid-commit or mid-write used to lose work with no warning.

The confirmation only shows up when there is a reason for it. Plain shells and idle agents close right away, no friction.
