...
| **A gate on a typed `claude`** | On Linux, `claude` with no arguments starts with the approval inbox and, inside a git repository in your home, a jail — see [Agent started in a shell](/en/agent/agent-in-a-shell). |
...
On Linux it goes beyond detecting: it defines a `claude` function in the session's shell, and that function is what makes a typed `claude` start **with a gate** instead of raw.
...
| **A gate on a typed `claude`** | Gone, on Linux. `claude` in a shell starts raw again, and TYBA only warns you. |
