...
Each item shows the risk, the command, the session and the folder — and clicking the session row takes you there. The buttons are the same as on the toast and in the agents queue: *Approve*, *Always allow* when the risk permits it, *Deny*, and *No, and tell it what to do*, which opens a field for telling the agent what to do instead. The decision is the same wherever you make it.

While the inbox is open, the toasts for the approvals it shows leave the screen: each request sits **in one place** for you to act on.
