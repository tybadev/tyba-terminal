...
| **Gate no `claude` digitado** | No Linux, `claude` sem argumentos sobe com inbox de aprovações e, num repositório git dentro da sua home, jaula — ver [Agente iniciado no shell](/pt-br/agente/agente-no-shell). |
...
No Linux ela vai além de detectar: define no shell da sessão uma função `claude`, e é essa função que faz o `claude` digitado subir **com gate** em vez de cru.
...
| **Gate no `claude` digitado** | Some, no Linux. `claude` num shell volta a subir cru, e o TYBA só avisa. |
