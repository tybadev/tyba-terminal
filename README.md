...
[![Sandbox](https://img.shields.io/badge/sandbox-Seatbelt%20%C2%B7%20bubblewrap-8b5cf6?style=flat-square)](#security-model)
...
- **An inbox, not N terminals to babysit.** Sessions report status (`Running`, `AwaitingInput`, `Idle`, `Exited`); you answer the one that is blocked without switching focus.
...
flowchart LR
    ...
    C --> D["<b>Monitor</b><br/>StatusDetector → inbox<br/>approve in place"]
...
**2. Worktree.** `git worktree add` off the current base — and the base SHA is **pinned right there**. Every diff you see later is `base_sha..HEAD` (three-dot semantics), so an agent that took two hours doesn't drown you in everything else that landed on `main` meanwhile. A per-repo `.tyba/setup.sh` hook runs here: symlink `.env`, `bun install`, per-worktree database.

**3. Spawn.** The PTY starts *inside* the sandbox, with an env built from a per-repo allowlist — never your shell's full environment, so `DATABASE_URL` and your tokens are simply absent. Writes are limited to the worktree plus temp; reads are deny-by-default, so `~/.ssh`, `~/.aws`, `tyba.db` and the neighboring worktrees are unreadable. Git refs are shared, but only the `refs/heads/tyba/` namespace is writable: an injected agent cannot `git update-ref` your local `main` and have your next push publish it.

**4. Monitor.** The StatusDetector prefers structured data — Claude Code's `--output-format stream-json` turns a pending tool use into a JSON event, with zero ANSI scraping — and falls back to OSC 133 shell integration plus a silence heuristic. A session that needs you becomes an inbox item with a native notification; you approve it there, and the answer is written straight into that PTY.
...
Closing the window kills nothing: state lives in the Rust core, so reopening the app reconnects you to the inbox with the agents still running.
...
| `git push`, `gh pr create` | Human approval, even in autonomous mode |
