...
- **Sandbox real no Linux** (PR #116): a mesma `SandboxSpec` traduzida para `bubblewrap` (bind mounts, `--unshare-net` onde cabe) + `seccomp`. Fail-closed igual: sem `bwrap` ou com user namespaces desabilitados, o spawn de agente é recusado com mensagem acionável — nunca degrada em silêncio.
  - **Login do agente no Linux: `~/.claude` é bind gravável, com sombras `--ro-bind` no que dá execução ou muda decisão de permissão** (`settings.json`, `settings.local.json`, `plugins/`, `hooks/`, `mcp.json`, `daemon.json`, `.config.json`, `commands/`, `skills/`, arquivos com cara de script). É o gêmeo Linux do keychain do macOS: com `~/.claude` somente-leitura, a credencial OAuth do Claude (gravada por `tmp`+`rename` atômico) batia em `EROFS` e a sessão nascia deslogada. Os mounts são ordenados por profundidade para a sombra funda sobreviver ao pai gravável, e `~/.claude.json` continua bind de arquivo (o `$HOME` nunca é montado inteiro). **Risco residual aceito**: com `.credentials.json` gravável, um agente injetado pode *substituir* o token do dono (ler já era possível) — escrita via tool passa pelo gate; escrita pelo processo é o que a promessa do login exige.
  - **`~/.claude/agents` é gravável de propósito, ao contrário de `hooks/`, `plugins/`, `daemon/`, `commands/` etc.** (fix/claude-agents-writable). O `/agents` do Claude Code (criar, editar, lockar uma definição de subagente) precisa escrever ali — com o diretório somente-leitura, todo o fluxo batia em `EROFS` dentro da jaula e funcionava normalmente fora dela (bug reportado, root-caused com evidência de bwrap real). A diferença que justifica destravar só este diretório: um `.md` de subagente é frontmatter + system prompt + tool allowlist — superfície de **prompt/privilégio** (prompt injection, permissão herdada), não execução de código no **host** como um hook ou um script de plugin. Read-only é a ferramenta certa pra fechar execução; é a ferramenta errada pra um arquivo que só é lido por um LLM. A mitigação escolhida é **visibilidade, não bloqueio** — o mesmo tratamento que `.claude.json`/`mcpServers` já recebem: o alarme de deriva (`agent::credentials`) ganhou um mecanismo dedicado (`agent_definition_drift`) que varre `agents/` **recursivamente** (não só o nível de topo — a subárvore inteira ficou gravável, e o scan não aposta em nenhuma suposição sobre até onde o Claude Code lê hoje) por identidade **caminho relativo + SHA-256 completo** (nunca um prefixo de hash — um prefixo curto seria força-brutável de dentro da jaula) e avisa o dono (evento `agent://sandbox-warning`, toast sticky) quando uma definição é **criada ou alterada, no PRÓXIMO SPAWN de sessão** — não só quando aparece um nome novo no topo de `~/.claude` (esse alarme, mais antigo, é raso demais pra pegar isto: `agents/` virou nome "conhecido" pra não gerar ruído a cada spawn só por existir, e o alarme de topo nunca olha dentro de um diretório). **Riscos residuais aceitos**: (1) um agente enjaulado pode plantar ou reescrever uma definição de subagente para uma sessão futura — a escrita em si não é bloqueada, só sinalizada; o dono decide o que fazer ao ver o aviso, o mesmo raciocínio já aceito para `.credentials.json` acima; (2) a checagem roda no **spawn**, não em vigilância contínua — um `.md` criado ou alterado **durante** a sessão N só surge no aviso da sessão N+1, então existe uma janela intra-sessão entre a escrita e o alerta.
