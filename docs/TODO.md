...
- **Texto digitado durante um comando é ecoado e GRAVADO no bloco.** A #236 fechou o caso visível — a seta em modo linha virava `^[[A` na saída persistida —, mas sobra digitar algo sensível enquanto um comando roda sem ler stdin: o driver do tty ecoa, e o eco entra no corpo do bloco que vai para o SQLite. Um prompt de senha **de verdade** desliga o `ECHO`, então esse caso não vaza; o que sobra é o descuido. Levantamento medido e opções no cofre (`tyba/features/terminal-blocks/eco-digitado-no-bloco`) — o ponto que sustenta o item é que o bloco é o **único** store que retém isso de forma durável: o `sessions.scrollback` guarda só a tela visível e é sobrescrito a cada 5s. Cai na regra 10 do CLAUDE.md e **merece decisão própria** — a saída fácil (segurar todo o teclado enquanto o tty está canônico) quebra o `y` de um `Ok to proceed?`, que é canônico com eco pelo mesmo motivo.
