  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-tyba-bg">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-2">
        <div className="flex items-center gap-2 rounded-[5px] border border-tyba-border px-2 py-1.5">
          <Robot size={14} className="shrink-0 text-tyba-text-faint" />
        ...
        {subagents.length === 0 ? (
          <p className="px-1 py-4 text-[12px] leading-relaxed text-tyba-text-faint">
            {t("agentsEmpty")}
          </p>
        ) : (
          <ul className="mt-1 flex flex-col gap-1 border-l border-tyba-border pl-2">
            {subagents.map((run, i) => {
              ...
              return (
                <li key={run.agent_id ?? `pending-${i}`}>
                  <button
                    type="button"
                    disabled={!clickable}
                    onClick={() => clickable && onSelect(run.agent_id as string)}
                    className={`flex w-full flex-col gap-0.5 rounded-[5px] border px-2 py-1.5 text-left ${
                      active
                        ? "border-tyba-green/40 bg-tyba-green/[.06]"
                        : "border-tyba-border"
                    } ${
                      clickable
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span
                        className={`size-1.5 shrink-0 rounded-full ${dotClass(run.status)}`}
                      />
                    ...
                    {run.status === "done" && run.summary && (
                      <p className="line-clamp-2 pl-[14px] text-[11px] leading-snug text-tyba-text-muted">
                        {run.summary}
                      </p>
                    )}
                  </button>
